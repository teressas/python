...
#********VALIDATIONS******************
    ...
    def validate_reg_users(data):
        ...
        if len(data['last_name']) < 2:
            errors['err_reg_last_name'] = "Last Name is required"

        if len(data['email']) < 1:
            errors['err_reg_email'] = "Email is required"
        elif not EMAIL_REGEX.match(data['email']):
            errors['err_reg_email'] = "Email is required"
        
            
        if len(data['password']) < 8:
            errors['err_reg_password'] = "Password is required"
        elif not PASSWORD_REGEX.match(data['password']):
            errors['err_reg_password'] = "Password is required"
            
        if len(data['confirm_password']) < 8:
            errors['err_reg_confirm_password'] = "Password is required"
