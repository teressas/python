...
# and creates a new list containing only the values from the original list that are greater than 
# its 2nd value. Print how many values this is and then return the new list. 
# If the list has less than 2 elements, have the function return False
    # Example: values_greater_than_second([5,2,3,2,1,4]) should print 3 and return [5,3,4]
    # Example: values_greater_than_second([3]) should return False
def values_greater_than_second(curr_list):
    newList = []
    for i in curr_list:
        if i >= curr_list[2]:
            newList.append(i)
...
def length_and_value(size:int, value:int):
    given_value = value
    newList = []
    newList.extend([given_value for i in range(size)])
    print(newList)
